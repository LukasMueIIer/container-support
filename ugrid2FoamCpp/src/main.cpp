...
struct FaceSignature {
    ...
    FaceSignature() : num_nodes(0), face_index(-1) {
        std::fill(sorted_nodes, sorted_nodes + 16, -1);
    }
...
public:
    KDTree(const std::vector<Point3D>& pts, const std::vector<int>& original_ids = {}) {
        if (!original_ids.empty() && original_ids.size() != pts.size()) {
            throw std::invalid_argument("KDTree: original_ids size must match pts size.");
        }
...
struct TagReader {
    ...
    bool load(const std::string& filename) {
        ...
        while (std::getline(file, line)) {
            ...
            // Format: ID Flag Name (e.g., "1 0 wallChamberRound")
            // This safely bypasses the header row since the header only has 1 integer.
            if (ss >> id >> flag) {
                // Safely grab the remainder of the line for the name (even if it has spaces)
...
void ensureOutward(int* face_nodes, int num_nodes, const Point3D& cell_center, const std::vector<Point3D>& coords) {
    ...
    if (normal.dot(center_to_face) < 0) {
        if (num_nodes == 3) {
            ...
            std::swap(face_nodes[1], face_nodes[3]);
        }
    }
}
...
std::string getOpenFOAMPatchType(const std::string& patchName) {
    ...
    if (lowerName.find("inlet") != std::string::npos) return "patch";
    if (lowerName.find("outlet") != std::string::npos) return "patch";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        for (size_t i = 0; i < ugrid.tris.size(); i += 3) {
            ...
            volSurfaceMap[FaceSignature(nodes, 3)] = ugrid.pids[i/3]; 
        }
        ...
        for (size_t i = 0; i < ugrid.quads.size(); i += 4) {
            ...
            volSurfaceMap[FaceSignature(nodes, 4)] = ugrid.pids[num_tris + i/4];
        }

        // Spatial Mapping (KDTree) fallback
        ...
        if (hasSurf && !surf.nodes.empty()) {
            ...
            for (const auto& tri : surf.tris) {
                ...
                spatialSurfMap[FaceSignature(n, 3)] = tri.surface_id;
            }
            for (const auto& quad : surf.quads) {
                ...
                spatialSurfMap[FaceSignature(n, 4)] = quad.surface_id;
            }
        }
        ...
        auto processCellFaces = [&](const std::vector<std::vector<int>>& local_faces, const Point3D& cell_center) {
            for(const auto& lf : local_faces) {
                ...
                f.owner = cellID;
                for(int i=0; i<f.num_nodes; ++i) f.nodes[i] -= 1; // 0-based OpenFOAM indexing
